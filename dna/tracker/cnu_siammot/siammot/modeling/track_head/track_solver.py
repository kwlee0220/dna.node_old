...
class TrackSolver(torch.nn.Module):
    ...
    def get_nms_boxes(self, detection):
        nms_idx = nms(detection.get('pred_boxes').tensor, detection.get('scores'), iou_threshold=0.4)

        detection = detection[nms_idx]

        # _ids = detection.get('ids')
        # _scores = detection.get('scores')

        # adjust the scores to the right range
        # _scores -= torch.floor(_scores) * (_ids >= 0) * (torch.floor(_scores) != _scores)
        # _scores[_scores >= 1.] = 1.

        detection.get('scores')[detection.get('scores') >= 2.] = detection.get('scores')[detection.get('scores') >= 2.] - 2.
        detection.get('scores')[detection.get('scores') >= 1.] = detection.get('scores')[detection.get('scores') >= 1.] - 1.

        return detection #, _ids, _scores

    def forward(self, detection: [Instances]):
        """
        ...
        all_ids = detection.get('ids')
        all_scores = detection.get('scores')
        ...
        combined_detection = self.get_nms_boxes(detection)
        _ids = combined_detection.get('ids')
        _state = combined_detection.get('state')
        _scores = combined_detection.get('scores')
        ...
        kill_ids = track_pool.expire_tracks()

        # _state = _ids
        for id in kill_ids:
            inactive_idxs[_ids == id] = False
            _state[_ids == id] = -2
        _state[inactive_idxs] = -1
