...
class EMM(torch.nn.Module):
    ...
    def forward(self, features, boxes, sr, targets=None, template_features=None):
        ...
        if self.training:
            features = tuple([features[f] for f in features])
        ...
        if self.training:
            ...
            src_bboxes = torch.cat([b.bbox.tensor for b in boxes], dim=0)
            gt_bboxes = torch.cat([b.bbox.tensor for b in targets], dim=0)
...
def get_scale_penalty(tlbr: torch.Tensor, boxes: Instances):
    box_w = boxes.pred_boxes.tensor[:, 2] - boxes.pred_boxes.tensor[:, 0]
    box_h = boxes.pred_boxes.tensor[:, 3] - boxes.pred_boxes.tensor[:, 1]
...
def wrap_results_to_boxlist(bb, bb_conf, boxes: [Instances], amodal=False):
    ...
    for _bb, _bb_conf, _boxes in zip(bb, bb_conf, boxes):
        _bb = _bb.reshape(-1, 4)
        track_box = Instances(_boxes.image_size)
        # track_box.set("bbox", Boxes(_bb))
        track_box.set("pred_boxes", Boxes(_bb))
        track_box.set("pred_classes", _boxes.get("pred_classes"))
        track_box.set("ids", _boxes.get("ids"))
        track_box.set("scores", _bb_conf)
...
def get_locations(fmap: torch.Tensor, template_fmap: torch.Tensor,
                  sr_boxes: [Instances], shift_xy, up_scale=1):
    """

    """
    h, w = fmap.size()[-2:]
    h, w = h*up_scale, w*up_scale
    if sr_boxes[0].has("bbox"):
        concat_boxes = torch.cat([b.bbox.tensor for b in sr_boxes], dim=0)
    else:
        concat_boxes = torch.cat([b.pred_boxes.tensor for b in sr_boxes], dim=0)
